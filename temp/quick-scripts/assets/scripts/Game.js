...
cc._RF.push(module, 'b64a6AdT6pHJ7IzuQfYpRDE', 'Game', __filename);
// scripts/Game.js
...
cc.Class({
    ...
    _setConstNum: function _setConstNum() {
    ...
    onTouchEnd: function onTouchEnd(event) {
        ...
        var rigidBody = this.mars.getComponent(cc.RigidBody);
        var center = rigidBody.getWorldCenter();
        //计算方向向量
    ...
    rankTapped: function rankTapped() {
        console.log('rank');
        this.failScane.active = false;
    ...
    rankStartTapped: function rankStartTapped() {
        this.rankScane.active = false;
        this.restart();
    },
    ...
    update: function update(dt) {
        ...
        if (this.marsBegan && !this.earth.onContact) {
            var marsVel = this.mars.getComponent(cc.RigidBody).linearVelocity;
        ...
        if (this.earth.onContact) {
            ...
            var vel = this.earth.getComponent(cc.RigidBody).linearVelocity;
            //cc.log(vel);
    ...
    updateForce: function updateForce() {
        var marsBody = this.mars.getComponent(cc.RigidBody);
        var earthBody = this.earth.getComponent(cc.RigidBody);
        var holePosition = this.blackHole.getComponent(cc.RigidBody).getWorldCenter();
    ...
    _updateUI: function _updateUI() {
        ...
        //适配墙壁
        var wallColliders = this.wall.getComponents(cc.PhysicsBoxCollider);
    ...
    _setBound: function _setBound(node, x, y, width, height) {
        node.offset.x = x;
        node.offset.y = y;
        node.size.width = width;
        node.size.height = height;
    },
    _setViewFullScreen: function _setViewFullScreen(node) {
        this._setFrame(node, 0, 0, this.node.width, this.node.height);
    },
    _setFrame: function _setFrame(node, x, y, width, height) {
        node.x = x;
        node.y = y;
        node.width = width;
        node.height = height;
